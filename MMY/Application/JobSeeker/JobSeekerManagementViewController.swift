//
//  JobSeekerManagementViewController.swift
//  MMY
//
...
class JobSeekerManagementViewController: FormViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        addLeftBarButton()
        title = "JobSeekingManagement".localized().uppercased()
        ...
        let suitableRow = LabelRow().cellSetup { (cell, row) in
            row.title = "ListOfJobs".localized()
        ...
        let postRow = LabelRow().cellSetup { (cell, row) in
            row.title = "RecruitmentPostManagement".localized()
            cell.imageView?.contentMode = .scaleAspectFit
            cell.imageView?.image = UIImage(named: "icon_write")
        }
        postRow.onCellSelection { (_, _) in
            let vc = FindJobManagerViewController(nibName: "FindJobManagerViewController", bundle: nil)
            self.navigationController?.pushViewController(vc, animated: true)
        }
